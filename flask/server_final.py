...
def crop_and_resize(image_cv2: np.ndarray, output_size=(640, 640)) -> np.ndarray:
    ...
    :param image_cv2: OpenCV 이미지 (ndarray)
    :param output_size: 최종 리사이즈할 크기 (width, height)
    :return: 크롭 후 리사이즈된 이미지
    """
    h, w, _ = image_cv2.shape
    # 짧은 변의 길이를 기준으로 정사각형 크기 결정
    side = min(h, w)
    # 이미지 중앙 좌표 계산
...
def obj_detection(model: YOLO, image_cv2: np.ndarray, conf_thres: float = 0.25,
    ...
    for r in results:
        for box in r.boxes:
            ...
            prediction_result = model.names[cls_id] if hasattr(model, "names") and model.names and cls_id in model.names else str(cls_id)
            detections.append({
