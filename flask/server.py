...
color_classification_model = YOLO(os.path.join(base_dir, "color_classification_test_224.pt"))  # size: 224
fruit_detection_model = YOLO(os.path.join(base_dir, "fruit_detection_test_v1.pt"))  # size: 640
animal_detection_model = YOLO(os.path.join(base_dir, "animal_detection_test_416.pt"))  # size: 416
...
def classify_center_region(model, image_cv2, resize=224):
    """
    이미지에서 9등분 영역(3x3) 중 '정중앙' 부분을 잘라 YOLOv8 분류 모델로 예측
